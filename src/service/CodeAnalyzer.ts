...
export class CodeAnalyzer implements ICodeAnalyzer {
	...
	constructor (private marshaller: IMarshaller,
		...
		this.stringUtil = new StringUtil(marshaller, typeDetector);
		this.nameGetter = new NameGetter(marshaller, typeDetector);
		...
		this.getAccessorFormatter = new GetAccessorFormatter(this.nameGetter, this.sourceFilePropertiesGetter, this.decoratorsFormatter, this.modifiersFormatter, this.parametersFormatter, this.valueableFormatter);
		this.setAccessorFormatter = new SetAccessorFormatter(this.nameGetter, this.sourceFilePropertiesGetter, this.decoratorsFormatter, this.modifiersFormatter, this.parametersFormatter, this.valueableFormatter);
		...
		this.classFormatter = new ClassFormatter(this.mapper, this.cache, this.decoratorsFormatter, this.propFormatter, this.methodFormatter, this.getAccessorFormatter, this.setAccessorFormatter, this.constructorFormatter, this.modifiersFormatter, this.valueableFormatter, this.heritageClauseFormatter, this.sourceFilePropertiesGetter);
	...
	public getFile (fileName: string): NodeArray<Statement>|null {
		...
		if (file == null) {
			if (this.fileLoader.existsSync(filePath)) {
				try {
					const content = this.fileLoader.loadSync(filePath).toString();
	...
	public getResolvedSerializedIdentifierValues (statements: (Statement|Expression|Node)[], deep: boolean = false): ResolvedSerializedIIdentifierValueMap {
		...
		Object.keys(unserialized.map).forEach(key => {
			...
			const type = this.typeDetector.getTypeof(value);

			switch (type) {
				case "object":
					map[key] = this.marshaller.marshal(value);
					break;
				...
				case "constructor":
					...
					staticKeys.forEach(staticKey => {
						...
						mappedKeys[staticKey] = this.marshaller.marshal(value);
					});
					map[key] = mappedKeys;
					break;
				...
				case "class":
					...
					staticKeysForClass.forEach(staticKey => {
						...
						mappedKeysForClass[staticKey] = this.marshaller.marshal(value);
					});
